...
## 4) React island demo (inside a real post)

This section proves the "Astro islands" approach:
- `ServerOnlyHello` renders via React on the server/build with **no** client hydration.
- `CounterIsland` hydrates with `client:visible`, so React is only downloaded when this component scrolls into view.
